...
public class LoginController {
	...
	public String naverLoginCallback(HttpServletRequest request) {
		String clientId = "UIsA65w26XgRrQkwfNJJ";
		String clientSecret = "3feoaDhxkb";
...
//				System.out.println(totalObj.get("access_token"));
				...
				String token = totalObj.get("access_token").getAsString();//정보접근을 위한 토글
...
//				System.out.println(responseBody);
				...
				//받아온 이메일이 데이터베이스의 email을 비교하여 가입유무를 확인 후
				//가입되어 있다면 로그인, 아니라면 회원가입창으로 정보를 가지고 이동
			}
	...
	private static String get(String apiUrl, Map<String, String> requestHeaders) {
		...
		try {
			...
			throw new RuntimeException("API 요청가 응답 실패 : " + apiUrl, e);
		} finally {
			conn.disconnect();
		}
	}
	
	//API에 연결하기 위한 HttpURLConnection객체를 생성하고 반환하는 메소드
